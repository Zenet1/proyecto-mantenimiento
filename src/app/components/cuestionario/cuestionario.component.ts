...
export class CuestionarioComponent implements OnInit {
  ...
  constructor(
    ...
    private servicioCuestionario: CuestionarioService,
  ...
  enviar() {
    if (
      ...
      for (let index = 0; index < this.preguntasForm.length; index++) {
        if (
          this.preguntasForm.controls[index].get('respuesta').value !=
          this.pregPrimarias[index].Respuesta
        ) {
          for (let j = 0; j < this.pregSecundariasForm.length; j++) {
            if (
              this.pregSecundarias[j].Enlace ==
              this.pregPrimarias[index].IDPregunta
            ) {
              ...
              if (
                ...
                if (
                  this.pregSecundariasForm.controls[j].get('respuesta').value !=
                  this.pregSecundarias[j].Respuesta
                ) {
                  cantidadIncorrecta++;
                }
              }
      ...
      if (cantidadIncorrecta > 0) {
        this.servicioCuestionario.rechazado().subscribe((respuesta) => {
          alert(
            'De acuerdo a tus respuestas, no es posible que asistas a la facultad, se te ha notificado por correo electrónico'
          );
          this.router.navigateByUrl('login');
        });
      } else {
